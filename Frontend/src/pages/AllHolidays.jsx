...
import { Search, Filter, Plus, RefreshCcw, Settings, Edit, Trash2, Loader2 } from 'lucide-react';
import AdminSidebar from '../components/AdminSidebar';
import TeacherSidebar from '../components/TeacherSidebar';
import Topbar from '../components/Topbar';

export default function AllHolidays() {
  const [activePage, setActivePage] = useState("all-holidays"); // eslint-disable-line no-unused-vars
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedData = data.map(h => ({
          ...
          type: h.holiday_type,
          createdAt: h.created_at ? new Date(h.created_at).toLocaleDateString() : '',
          updatedAt: h.updated_at ? new Date(h.updated_at).toLocaleDateString() : '',
          recurring: 'No',
          status: 'Active'
        }));
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex items-center space-x-2 text-gray-500 mb-6 text-sm">
            <span>🏠 Front</span>
            <span>/</span>
            <span className="text-gray-900 font-medium">Holiday Management</span>
          </div>
              ...
              <table className="min-w-full text-sm whitespace-nowrap">
                <thead className="bg-gray-100 text-gray-600 uppercase text-xs">
                  <tr>
                    ...
                    <th className="p-3 text-left w-20">Start Date</th>
                    <th className="p-3 text-left w-20">End Date</th>
                    ...
                    <th className="p-3 text-left w-20">Created At</th>
                    <th className="p-3 text-left w-20">Updated At</th>
                    <th className="p-3 text-left w-16">Recurring</th>
                ...
                <tbody>
                  {holidays.map((item, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3 text-gray-700">{item.createdAt}</td>
                      <td className="p-3 text-gray-700">{item.updatedAt}</td>
                      <td className="p-3 text-gray-700">{item.recurring}</td>
                      <td className="p-3">
